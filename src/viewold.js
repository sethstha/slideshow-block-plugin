	...
	try {
		...
		for (let i = 0; i < number; i++) {
			container.insertAdjacentHTML(
				'afterbegin',
				`<button type="button" class="sethstha-pagination-indicator" aria-label="Navigate to slide ${i}" aria-selected="false"></button>`
			);
		}
	} catch (error) {
		console.error(error);
...
document.addEventListener('DOMContentLoaded', function () {
	...
	const paginations = document.querySelectorAll(
		'.sethstha-pagination-indicator'
	);
	paginations.forEach(function (i) {
